...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════════
    # PART 2: SENSITIVITY — Strata count variation (primary DV only)
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*70}")
    ...
    for sg_name, sg_mask in subgroups:
        ...
        for K in K_VALS:
            ...
            strata_rows.append({
                "Subgroup": sg_name, "K": K, "ATT": round(att, 4),
    ...
    # ══════════════════════════════════════════════════════════════════════
    # PART 3: SENSITIVITY — PS trimming (primary DV, K=5)
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*70}")
    print("  PART 3: SENSITIVITY — PS trimming (primary DV)")
    print(f"{'='*70}")
    ...
    for sg_name, sg_mask in subgroups:
        ...
        for lo, hi in TRIMS:
            ...
            trim_rows.append({
                "Subgroup": sg_name, "Trim": f"[{lo},{hi}]", "N": n_keep,
                "ATT": round(att, 4), "CI_lo": round(ci_lo, 4), "CI_hi": round(ci_hi, 4),
                "Sig": sig,
            })
    ...
    for ax_i, sg in enumerate(key_sg):
        ax = axes[ax_i]
        sg_rows = strata_df[strata_df["Subgroup"] == sg]
        if sg_rows.empty:
