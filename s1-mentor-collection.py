...
Checkpoint files (resume-safe):
  _checkpoint_revisions_wikitext.jsonl  — raw wikitext revisions
  _checkpoint_revisions_json.jsonl      — raw JSON revisions
  _checkpoint_uid_map.json              — username→user_id cache

Output:
  s1_mentor_list.jsonl   — one row per mentor, complete timeline
  s1_mentor_change.jsonl — every state change event
"""
...
CKPT_WIKI = os.path.join(BASE, "s1_checkpoint_revisions_wikitext.jsonl")
CKPT_JSON = os.path.join(BASE, "s1_checkpoint_revisions_json.jsonl")
CKPT_UID  = os.path.join(BASE, "s1_checkpoint_uid_map.json")
...
# ─── Fetch revisions from API with checkpoint ───
...
# ─── Username → user_id resolution with cache ───

def load_uid_cache():
    if not os.path.isfile(CKPT_UID):
        return {}
...
# ─── Replay wikitext revisions → change events ───
...
# ─── Replay JSON revisions → change events ───
...
def main():
    ...
    list_path = os.path.join(BASE, "s1_mentor_list.jsonl")
    ...
    change_path = os.path.join(BASE, "s1_mentor_change.jsonl")
    ...
    n_wiki_only = sum(1 for m in mentors if m["sources"] == ["wikitext"])
    n_json_only = sum(1 for m in mentors if m["sources"] == ["json"])
    n_both = sum(1 for m in mentors if "wikitext" in m["sources"] and "json" in m["sources"])
    n_ever_auto = sum(1 for m in mentors if m["ever_auto"])
    n_ever_manual = sum(1 for m in mentors if m["ever_manual"])
    n_both_types = sum(1 for m in mentors if m["ever_auto"] and m["ever_manual"])
    n_auto_only = sum(1 for m in mentors if m["ever_auto"] and not m["ever_manual"])
    n_manual_only = sum(1 for m in mentors if m["ever_manual"] and not m["ever_auto"])
    n_cur_auto = sum(1 for m in mentors if m["current_pool_status"] == "auto")
    n_cur_manual = sum(1 for m in mentors if m["current_pool_status"] == "manual")
    ...
    print(f"  Wikitext-only:        {n_wiki_only}")
    print(f"  JSON-only:            {n_json_only}")
    print(f"  Both periods:         {n_both}")
    print(f"\nPool status (lifetime):")
    print(f"  Ever auto (weight>=1):  {n_ever_auto}")
    print(f"  Ever manual (weight=0): {n_ever_manual}")
    print(f"  Both auto & manual:     {n_both_types}  (switched between the two)")
    print(f"  Auto only (never w=0):  {n_auto_only}")
    print(f"  Manual only (never w>0):{n_manual_only}")
    print(f"\nCurrent active pool status:")
    print(f"  auto (weight>=1):       {n_cur_auto}")
    print(f"  manual (weight=0):      {n_cur_manual}")
    print(f"\nTotal change events:    {len(change_log)}")
