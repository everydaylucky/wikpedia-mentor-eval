#!/usr/bin/env python3
"""
s10_1_perspective_api.py — Fetch Perspective API scores for mentee questions + mentor replies.
...
Usage:
  python s10_1_perspective_api.py                    # process all first convs
  python s10_1_perspective_api.py --auto-only        # only auto-assigned mentors
  python s10_1_perspective_api.py --type mentee      # mentee questions only
  python s10_1_perspective_api.py --type mentor      # mentor replies only
  python s10_1_perspective_api.py --limit 1000       # process N then stop
"""
...
def load_mentor_timelines():
    ...
    with open(S1_FILE, encoding="utf-8") as f:
        for line in f:
            ...
            timelines[m["username"]] = periods
    return timelines


def get_mentor_type(mentor, timestamp, timelines):
    """weight>=1 → auto, weight=0 → manual (same logic as s9)."""
    tl = timelines.get(mentor, [])
    if not tl:
        return "unknown"
    try:
        conv_dt = datetime.datetime.fromisoformat(
            timestamp.replace("Z", "+00:00")).replace(tzinfo=None)
...
def main():
    ...
    ap.add_argument("--auto-only", action="store_true",
                    help="Only process auto-assigned mentor conversations")
    args = ap.parse_args()
    ...
    if args.auto_only:
        timelines = load_mentor_timelines()
        print(f"  Loaded {len(timelines):,} mentor timelines")
        before = len(conversations)
        conversations = [
            r for r in conversations
            if get_mentor_type(r["mentor"], r["timestamp"], timelines) == "auto"
        ]
        print(f"  Auto-only filter: {before:,} → {len(conversations):,}")
